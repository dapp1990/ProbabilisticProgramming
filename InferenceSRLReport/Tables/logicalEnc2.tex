\begin{table}[htb]
\centering
\caption{Indicator variables using ENC2}
\label{logicalEnc2}
\begin{tabular}{c|c|c}
\hline
Variables & Indicator Variable                                  & CTP                                                                                                                                                                    \\ \hline
B         & \(\lambda_{b1}\), \(\lambda_{b2}\)                  & \(\rho_{b1}\)                                                                                                                                                          \\ \hline
E         & \(\lambda_{e1}\), \(\lambda_{e2}\), \(\lambda_{3}\) & \(\rho_{e1}\), \(\rho_{e2}\)                                                                                                                                           \\ \hline
A         & \(\lambda_{a1}\), \(\lambda_{a2}\)                  & \begin{tabular}[c]{@{}c@{}}\(\rho_{a1|b1,c1}\), \(\rho_{a1|b1,c2}\), \(\rho_{a1|b1,c3}\),\\ \(\rho_{a1|b2,c1}\), \(\rho_{a1|b2,c2}\), \(\rho_{a1|b2,c3}\)\end{tabular} \\ \hline
J         & \(\lambda_{j1}\), \(\lambda_{j2}\)                  & \(\rho_{j1|a1}\), \(\rho_{j1|a2}\)                                                                                                                                     \\ \hline
M         & \(\lambda_{m1}\), \(\lambda_{m2}\)                  & \(\rho_{m1|a1}\), \(\rho_{m1|a2}\)                                                                                                                                    
\end{tabular}
\end{table}